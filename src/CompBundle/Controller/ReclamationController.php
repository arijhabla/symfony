...
use CompBundle\Entity\Reclamation;
...
class ReclamationController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $reclamations = $em->getRepository('CompBundle:Reclamation')->findAll();
    ...
    public function newAction(Request $request)
    {
        ...
        $form = $this->createForm('CompBundle\Form\ReclamationType', $reclamation);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $pp=$reclamation->getIdEmploye()->getId();
            $reclamation->setIde($pp);
            $em = $this->getDoctrine()->getManager();
     ...
     *
     * @Route("/front/{id}", name="reclamation_front")
     * @Method("GET")
     */
    public function frontAction(Request $request)
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $query1 = $em->createQuery('SELECT n FROM CompBundle:Reclamation n WHERE n.ide=:Men ')->setParameter(':Men',$input);
        $reclamations = $query1->getResult();
        return $this->render('reclamation/front.html.twig', array(
            'reclamations' => $reclamations,
        ));
    }
    ...
    {
        $deleteForm = $this->createDeleteForm($reclamation);
        $editForm = $this->createForm('CompBundle\Form\ReclamationType', $reclamation);
