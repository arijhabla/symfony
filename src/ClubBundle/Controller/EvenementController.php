...
class EvenementController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $evenements = $em->getRepository('ClubBundle:Evenement')->findAll();
    ...
    public function newAction(Request $request)
    {
        ...
        $form = $this->createForm('ClubBundle\Form\EvenementType', $evenement);
     ...
     *
     * @Route("/front/{id}", name="evenement_participer")
     * @Method("GET")
     */
    public function participerAction(Evenement $evenement)
    {
        $evenement->setCapacity($evenement->getCapacity()-1);
        $em = $this->getDoctrine()->getManager();
        $clubs = $em->getRepository('ClubBundle:Club')->findAll();

        $em->persist($evenement);
        $em->flush();
        return $this->render('club/front.html.twig', array(
            'evenement' => $evenement,
            'clubs' => $clubs,
        ));
    }
    ...
    {
        $deleteForm = $this->createDeleteForm($evenement);
        $editForm = $this->createForm('ClubBundle\Form\EvenementType', $evenement);
